...
const Projects = () => {
    ...
    const nextProject = () => {
        setCurrentIndex((prevIndx) => (prevIndx + 1) % projectsData.length);
    }

    const prevProject = () => {
        setCurrentIndex((prevIndx) => prevIndx === 0 ? projectsData.length - 1 : prevIndx - 1);
    }
            ...
            <div className='overflow-hidden'>
                <div className='flex gap-8 transition-transform duration-500 ease-in-out' style={{ transform: `translateX(-${(currentIndex * 100) / cardsToShow}%)` }}>
                    {projectsData.map((project, index) => (
                        <div className='relative flex-shrink-0 sm:1/4' key={index}>
